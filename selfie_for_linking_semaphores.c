...
void saveMemory(int* processToSave);
...
void restoreMemory(int* processToSave);
void restoreRegisters(int* processToSave);
int getMemoryBase(int* currentProcess);
int getMemoryBound(int* currentProcess);
...
void copyProcess(int* nextFreeProc);
void copyState(int* nextFreeProc);

int runUntilExit();

int boot(int argc, int* argv);

int selfie_run(int debugger);
...
void implementRead() {
  ...
  vaddr = addressTranslation(vaddr);
  fd    = *(registers+REG_A0);
...
void implementFork(){
  ...
  nextFreeProc = getNextFreeProc();
  if((int) nextFreeProc != -1){
    copyProcess(nextFreeProc);
    copyState(nextFreeProc);
    *(registers + REG_V0) = getId(nextFreeProc);
  } else {
    *(registers + REG_V0) = 0;
    print((int*)"No free memory!");
    println();
  }
...
void implementId(){
  *(registers + REG_V0) = getId(currentProcess);
}
...
void implementLock(){
  ...
  currentPc = pc - WORDSIZE - getMemoryBase(currentProcess);
...
int addressTranslation(int vaddr) {
  return vaddr + getMemoryBase(currentProcess);
}

int isValidAddress(int vaddr) {
  if (vaddr >= getMemoryBase(currentProcess))
    if (vaddr <= getMemoryBound(currentProcess))
...
void initProcessTable() {
  ...
  currentBase = 0;
  currentBound = sizeOfProcesses;
  procCount = 1;
  ENTRY_SIZE = 43 * WORDSIZE;

  while(procCount <= numberOfProcesses){
    if(procCount == 1) {
      processTable = malloc(ENTRY_SIZE);
      currentProcess = processTable;
      setTrap(currentProcess, 0);
    } else { // malloc for ech process eventhough they are not forked yet
      temp = malloc(ENTRY_SIZE);
      setNextProcess(currentProcess, temp);
      currentProcess = temp;
      setTrap(currentProcess, 1);
    }
    setLock(currentProcess, 0);
    setWaitingFor(currentProcess, 0);
    setId(currentProcess, procCount);
    saveBrk(currentProcess);
    saveRegisters(currentProcess);
    setMemoryBaseBound(currentProcess, currentBase, currentBound);
    currentBase = currentBound;
    currentBound = currentBound + sizeOfProcesses;
    procCount = procCount + 1;
  }
  setNextProcess(currentProcess, processTable); //last entry pointer points to first entry
  currentProcess = processTable;
}
...
void setMemoryBaseBound(int* currentProcess, int start, int end){
  start = start * MEGABYTE;
  end = end * MEGABYTE - 4;
  *(currentProcess + 4) = start;
...
void saveRegisters(int* processToSave){
  int regCount;
  regCount = 0;
  while(regCount < NUMBEROFREGISTERS){
...
void saveMemory(int* processToSave){
  int memCount;
  memCount = 0;
  while(memCount < memorySize / WORDSIZE){
    *(processToSave + 9 + NUMBEROFREGISTERS + 2 + memCount) = *(memory + memCount);
    memCount = memCount + 1;
  }
}
...
int getMemoryBase(int* currentProcess){
  return *(currentProcess + 4);
}

int getMemoryBound(int* currentProcess){
  return *(currentProcess + 5);
}
...
void restoreMemory(int* processToSave){
  ...
  memCount = 0;
  while(memCount < memorySize / WORDSIZE){
    *(memory + memCount) = *(processToSave + 9 + NUMBEROFREGISTERS + 2 + memCount);
    memCount = memCount + 1;
  }
}
...
void op_jal() {
  ...
  if (interpret) {
    ...
    *(registers+REG_RA) = pc + 8;
    ...
    pc = addressTranslation(instr_index * WORDSIZE);
  ...
  if (debug) {
    ...
    if (sourceLineNumber != (int*) 0) {
      ...
      if(numberOfProcesses > 1)
        printInteger(*(sourceLineNumber + (pc - getMemoryBase(currentProcess)) / WORDSIZE));
      else
...
void switchProcess() {
  ...
  if(globalTrap() != -1) {
    ...
    restoreState();
    if(debug){
      printInteger(getId(currentProcess));
      println();
    }
  }
}
...
int* getNextFreeProc(){
  ...
  nextProcess = getNextProcess(currentProcess);

  while(getTrap(nextProcess) == 0) {
    count = count + 1;
...
void copyProcess(int* nextFreeProc) {
  ...
  counterMemoryCells = 0;
    ...
    while((counterMemoryCells + getMemoryBase(nextFreeProc) / 4) <= (getMemoryBound(nextFreeProc) / 4)) {
      *(memory + counterMemoryCells + getMemoryBase(nextFreeProc) / 4) = *(memory + counterMemoryCells + getMemoryBase(currentProcess) / 4);
      counterMemoryCells = counterMemoryCells + 1;
    }
}

void copyState(int* nextFreeProc){
  saveState(nextFreeProc);

  *(nextFreeProc + 2) = pc - getMemoryBase(currentProcess) + getMemoryBase(nextFreeProc);
  setTrap(nextFreeProc, 0);
  *(nextFreeProc + 9 + REG_V0) = 0;
  *(nextFreeProc + 9 + REG_RA) = *(registers + REG_RA) - getMemoryBase(currentProcess) + getMemoryBase(nextFreeProc);
}
...
int boot(int argc, int* argv) {
  ...
  SP = sizeOfProcesses * MEGABYTE - WORDSIZE;
  SP = up_loadArguments(argc, argv, SP);
...
int selfie_run(int debugger) {
  ...
  sizeOfProcesses = atoi(getArgument());
  numberOfProcesses = atoi(peekArgument());

  initMemory(sizeOfProcesses * numberOfProcesses);
